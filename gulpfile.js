...
//-----JADE-------
gulp.task('jade', ['mergeJSON'], function() {
    return gulp
        .src(['./develop/jade/includes/*.pug', './develop/jade/*.pug'])
        .pipe(
            changed(app, {
                extension: '.html'
            })
...
gulp.task('mergeJSON', function() {
    return gulp
        .src(['./develop/blocks/**/*.json', './develop/page_data/**/*.json'])
        .pipe(
            merge({
...
gulp.task('jadeBlocks', ['mergeJSON'], function() {
    return gulp
        .src([
            './develop/jade/**/*.pug',
            './develop/jade/template/**/*',
            '!./develop/jade/includes/**/*',
            '!./develop/jade/includes.pug'
        ])
        .pipe(changed(app))
        .pipe(plumber())
        .pipe(pugGlobbing())
...
gulp.task('watch', function() {
    gulp.watch(
        'images/{' + sprites.join(',') + '}/*.{jpg,png,svg,gif}',
        {
            cwd: 'develop'
        },
        ['sprite']
    ...
    gulp.watch(
        ['jade/includes/*.pug', 'jade/*.pug'],
        {
            cwd: 'develop'
        },
        ['jadeBlocks', browserSync.reload]
    ); //шаблоны
...
gulp.task('build:copyDist', function() {
    return gulp
        .src([app + '/*.php', app + '/.htaccess', app + '/favicon.png'])
        .pipe(print())
        .pipe(gulp.dest('dist'));
});
...
gulp.task('build:copyDistFonts', function() {
    return gulp
        .src([app + '/fonts/**/{*.eot,*.svg,*.ttf,*.eot,*.otf,*.woff2,*.woff}'])
        .pipe(print())
...
gulp.task('build:minifiImg', function() {
    return gulp
        .src([app + '/images/**/{*.jpg,*.png,*.jpeg,*.gif}'])
        .pipe(print())
        .pipe(
            imagemin({
                zopflipng: false
